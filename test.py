...
comp_sector = get_the_value(max(revs), sectors)  # Определение сектора рынка
dct = get_dct(comp_sector, MBx11, CBx11, KBx11)  # Присвоение нужного словаря
x11 = get_the_value(age_of_comp, dct)
coef_of_var = (k_variable(revs)) * 1000  # Определение коэффициента вариации * 1000
dct = get_dct(comp_sector, MBx12, CBx12, KBx12)  # Присвоение нужного словаря
x12 = get_the_value(round(coef_of_var), dct)
x1 = ((x11 + x12) / 2) * 0.125  # Определение фактора x1 "История"

# Расчет фактора x2 "Средства"
dct = get_dct(comp_sector, MB1x21, MB2x21, MB3x21, CBx21, KBx21)
x21 = get_the_value(mtb, dct)
res = round((comp_capital / total_assets), 2) * 100  # Расчет коэф автономии (капитал / общую сумму всех активов)
...
z1 = (get_middle_exp(senior_mngr_cnt, h_edu_senior, m_edu_senior, non_edu_senior)) * 100
x1_31 = get_the_value(z1, b7)
x2_31 = get_the_value(exp_senior, b8)
x31 = (x1_31 + x1_31) * 0.5
x1_32 = get_the_value(exp_middle, b8)
a = get_middle_exp(middle_mngr_cnt, h_edu_middle, m_edu_middle, non_edu_middle)
