...
"""Имена переменных x* взяты из ГОСТ Р 66.1.01-2015 'ОЦЕНКА ОПЫТА И ДЕЛОВОЙ РЕПУТАЦИИ СУБЪЕКТОВ 
ПРЕДПРИНИМАТЕЛЬСКОЙ ДЕЯТЕЛЬНОСТИ ЛИЦ,ОСУЩЕСТВЛЯЮЩИХ АРХИТЕКТУРНО-СТРОИТЕЛЬНОЕ ПРОЕКТИРОВАНИЕ'"""

"""Расчет фактор  x1 'История'
1. Определяем сектор экономики в котором находится заявитель по максимальному элементу revs,
2. Получаем словарь для поиска значения по параметру comp_sector,
3. Определяем субфактор x11: 'Количество лет на рынке'.
"""
comp_sector = get_the_value(max(revs), sectors)
dct = get_dct(comp_sector, MBx11, CBx11, KBx11)
x11 = get_the_value(age_of_comp, dct)
"""
4. Расчитываем коэффициент вариации
5. Получаем словарь по параметру 'comp_sector'
6. Определяем субфактор x12: 'Ритмичность работы' по словарю dct, округляем чтоб получить int
"""
coef_of_var = (k_variable(revs))
dct = get_dct(comp_sector, MBx12, CBx12, KBx12)
x12 = get_the_value(round(coef_of_var), dct)
"""
7. Расчитываем фактор x1 'История'
"""
x1 = ((x11 + x12) / 2) * 0.125
